/**
 * Appendix A - Reasonable Adjustment Strategies Matrix.
 * Must match src/lib/appendixAMatrixData.ts structure for consistent checkbox IDs.
 */
...
function cb(checked: Record<string, boolean>, id: string): string {
  const c = checked[id];
  return `<span class="appendix-cb${c ? ' checked' : ''}">${c ? '✓' : ''}</span>`;
}

export function renderAppendixAMatrixHtml(checked: Record<string, boolean>): string {
  let html = '<table class="appendix-matrix-table"><thead><tr><th>Category</th><th>Possible Issue</th><th>Reasonable Adjustment Strategy (select as applicable)</th></tr></thead><tbody>';
  for (const row of APPENDIX_A_TABLE1) {
    const catHtml = `<div class="appendix-cell-item">${cb(checked, row.category.id)} ${row.category.label}</div>`;
    const issuesHtml = row.issues.map((i) => `<div class="appendix-cell-item">${cb(checked, i.id)} ${i.label}</div>`).join('');
    const stratHtml = row.strategies.map((s) => `<div class="appendix-cell-item">${cb(checked, s.id)} ${s.label}</div>`).join('');
    html += `<tr><td>${catHtml}</td><td>${issuesHtml}</td><td>${stratHtml}</td></tr>`;
  }
  html += '</tbody></table>';
  ...
  for (const row of APPENDIX_A_TABLE2) {
    const catHtml = row.categoryItems.map((c) => `<div class="appendix-cell-item">${cb(checked, c.id)} ${c.label}</div>`).join('');
    const stratHtml = row.strategies.map((s) => `<div class="appendix-cell-item">${cb(checked, s.id)} ${s.label}</div>`).join('');
    html += `<tr><td>${catHtml}</td><td>${stratHtml}</td></tr>`;
  }
  html += '</tbody></table>';
  return html;
}
