  ...
  return (
    <div className={cn('mb-4 sm:mb-6 md:mb-8', className)}>
      ...
      <div className="hidden sm:block">
        <div className="flex items-start justify-between overflow-x-auto pb-4 scrollbar-hide -mx-2 sm:mx-0 px-2 sm:px-0">
          {steps.map((step, index) => {
            const isActive = step.number === currentStep;
            const isCompleted = step.number < currentStep;
            const isLast = index === steps.length - 1;
              ...
              <React.Fragment key={step.number}>
                <div className="flex items-center flex-1 min-w-0">
                  ...
                  <div className="flex flex-col items-center flex-shrink-0">
                    ...
                    >
                      {isCompleted ? <Check className="w-5 h-5" strokeWidth={3} /> : step.number}
                    </div>
                    {/* Step Label */}
