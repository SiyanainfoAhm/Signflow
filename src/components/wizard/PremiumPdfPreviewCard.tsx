...
export const PremiumPdfPreviewCard: React.FC<PremiumPdfPreviewCardProps> = ({ role }) => {
  ...
  return (
    <Card className="overflow-hidden" padding="none">
      {/* Header */}
      <div className="bg-gradient-to-r from-[var(--brand)] to-[#E06A0F] p-3 sm:p-4">
        <div className="flex items-center justify-between gap-2">
          <div className="flex-1 min-w-0">
            <h3 className="text-white font-bold text-sm sm:text-base">PDF Preview</h3>
            <p className="text-orange-100 text-[10px] sm:text-xs mt-0.5 line-clamp-1">
              {isOutdated ? 'Preview may be outdated - click Preview to update' : 'Click Preview to generate PDF'}
            </p>
          </div>
          {isOutdated && (
            <div className="bg-orange-500/20 border border-orange-300/50 rounded px-1.5 sm:px-2 py-0.5 sm:py-1 flex-shrink-0">
              <span className="text-orange-100 text-[9px] sm:text-[10px] font-medium">Outdated</span>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Controls */}
      <div className="p-2 sm:p-3 bg-gray-50 border-b border-[var(--border)] space-y-2 sm:space-y-3">
        ...
        <div className="flex items-center justify-between gap-1 sm:gap-2 flex-wrap">
          <div className="flex items-center gap-0.5 sm:gap-1">
            <Button
              ...
              disabled={zoom <= 0.25}
              className="p-1 sm:p-1.5 h-7 sm:h-8 w-7 sm:w-8"
            >
              <ZoomOut className="w-3.5 sm:w-4 h-3.5 sm:h-4" />
            </Button>
            <span className="text-[10px] sm:text-xs font-medium text-gray-700 min-w-[40px] sm:min-w-[50px] text-center">
              {Math.round(zoom * 100)}%
            </span>
            <Button
              variant="ghost"
          ...
          <div className="flex items-center gap-0.5 sm:gap-1">
            ...
            <span className="text-[10px] sm:text-xs font-medium text-gray-700 min-w-[50px] sm:min-w-[60px] text-center">
        ...
        <div className="flex gap-1.5 sm:gap-2">
          <Button
            ...
            disabled={loading}
            size="sm"
            className="flex-1 flex items-center justify-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs px-2 sm:px-3"
          >
            {loading ? (
              <>
          ...
          <Button
            ...
            size="sm"
            className="flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs px-2 sm:px-3"
          >
      ...
      <div className="p-2 sm:p-4 bg-[var(--bg)] h-[300px] sm:h-[400px] md:h-[520px] flex items-center justify-center overflow-hidden relative">
        {loading ? (
          ...
          <div className="text-center text-red-600 px-4">
            <p className="text-[10px] sm:text-xs font-medium mb-1 sm:mb-2">Error generating preview</p>
            <p className="text-[9px] sm:text-[10px] text-red-400 break-words">{error}</p>
            <Button
              variant="outline"
              size="sm"
              onClick={() => generatePdf()}
          ...
          <div className="w-full h-full flex justify-center items-center overflow-auto">
            <div className="bg-white p-1 sm:p-2 rounded border border-[var(--border)] shadow-inner">
              <iframe
                ref={iframeRef}
                ...
                style={{
                  width: `${Math.min(595.28 * zoom, window.innerWidth - 40)}px`,
                  height: `${Math.min(841.89 * zoom, window.innerHeight * 0.4)}px`,
                  maxWidth: '100%',
                }}
                title="PDF Preview"
              />
          ...
          <div className="text-center text-gray-500 px-4">
            <p className="text-[10px] sm:text-xs font-medium">Preview will appear here</p>
            <p className="text-[9px] sm:text-[10px] mt-1 sm:mt-1.5 text-gray-400">
