...
export const PremiumLikertCardList: React.FC<PremiumLikertCardListProps> = ({ section, errors = {} }) => {
  ...
  return (
    <Card className="mb-4 sm:mb-6">
      ...
      {/* Legend */}
      <div className="mb-4 sm:mb-6 p-2 sm:p-3 bg-gray-50 rounded-lg border border-gray-200 overflow-x-auto">
        <div className="flex items-center justify-between text-[10px] sm:text-xs font-medium text-gray-600 gap-2 min-w-max sm:min-w-0">
          <span className="whitespace-nowrap">1 - {scaleLabels[0]}</span>
          <span className="whitespace-nowrap">2 - {scaleLabels[1]}</span>
          <span className="whitespace-nowrap">3 - {scaleLabels[2]}</span>
          <span className="whitespace-nowrap">4 - {scaleLabels[3]}</span>
          <span className="whitespace-nowrap">5 - {scaleLabels[4]}</span>
        </div>
      </div>
        ...
        {section.questions.map((question) => {
          ...
          return (
            <div
              ...
              className={`p-3 sm:p-4 md:p-5 bg-white border rounded-lg transition-all ${
                error
                  ? 'border-red-300 bg-red-50'
                  : 'border-[var(--border)] hover:border-gray-300 hover:shadow-sm'
              }`}
            >
              <p className="text-xs sm:text-sm font-medium text-[var(--text)] mb-3 sm:mb-4 leading-relaxed">
                {question.question}
              </p>
              <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                {scaleValues.map((value) => {
                  ...
                  return (
                    <label
                      key={value}
                      className={`relative flex-1 min-w-[60px] sm:min-w-[70px] cursor-pointer group touch-manipulation ${
                    ...
                    >
                      ...
                      />
                      <div
                        className={`relative px-2 sm:px-3 py-2 sm:py-2.5 rounded-lg border text-center transition-all duration-200 ${
                          isSelected
                            ? 'border-[var(--brand)] bg-orange-50 text-[var(--brand)] font-semibold shadow-sm'
                            : 'border-[var(--border)] bg-white text-gray-700 hover:border-orange-300 hover:bg-orange-50 active:bg-orange-100'
                        } ${!canEdit ? 'hover:border-[var(--border)] hover:bg-white' : ''}`}
                      >
                        <div className="flex items-center justify-center gap-1 sm:gap-1.5">
                          <div
                            className={`w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${
                              isSelected
                        ...
                        <div className="mt-0.5 sm:mt-1 text-[8px] sm:text-[9px] text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity truncate hidden sm:block">
                          {label}
                        </div>
                      </div>
                    </label>
              ...
              {error && <p className="mt-2 sm:mt-3 text-xs sm:text-sm text-red-600 font-medium">{error}</p>}
            </div>
          );
        })}
      </div>
